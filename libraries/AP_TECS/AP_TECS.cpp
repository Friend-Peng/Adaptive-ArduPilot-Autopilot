...
float AP_TECS::_update_throttle_adaptive_robust_rule(float pid_sum, float error, float error_dot, float error_int)
{
	...
	float intK0_delta = (norm_s - _asmc_thrAlfa * _intK0Thr) * _DT;
	float intK1_delta = (norm_s * norm_xi - _asmc_thrAlfa * _intK1Thr) * _DT;
	float intK2_delta = (norm_s * norm_xi*norm_xi - _asmc_thrAlfa * _intK2Thr) * _DT;
...
void AP_TECS::_update_throttle_with_airspeed(void)
{
    ...
    else
    {
		...
		if ( _switch == 1 )
		{
			...
			float e = (_STE_error * K_STE2Thr + 0)      * fabs(p1 * p2);
...
float AP_TECS::_update_pitch_adaptive_robust_rule(float pid_sum, float error, float error_dot, float error_int)
{
	...
	float intK0_delta = (norm_s - _asmc_ptchAlfa * _intK0Pitch) * _DT;
	float intK1_delta = (norm_s * norm_xi - _asmc_ptchAlfa * _intK1Pitch) * _DT;
	float intK2_delta = (norm_s * norm_xi*norm_xi - _asmc_ptchAlfa * _intK2Pitch) * _DT;
